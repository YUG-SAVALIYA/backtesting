...
for sym in candidate_symbols:
    ...
    # -------------------------------------------------------------
    # LIVE & BACKTESTER MATCHED WITH DAILY FALLBACK FOR INCOMPLETE 5M
    # -------------------------------------------------------------
    sig_df_live, exec_df_live = live_strat.prepare_frames(df_sym.copy(), df_sym.copy(), htf_df)
    sigs_live = live_strat.generate_signals(sym, sig_df_live, exec_df_live, target_level=17.0)
    ...
    for sig in valid_sigs_live:
        ...
        all_bt_trades.append(trade_payload)
        all_live_trades.append(trade_payload)

df_bt = pd.DataFrame(all_bt_trades)
df_live = pd.DataFrame(all_live_trades)

print("\n================ FULL SIDE-BY-SIDE MATCH RESULTS (WITH DAILY FALLBACK IN BOTH) ================")
