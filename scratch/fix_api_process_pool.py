...
# ----------------- MODULE LEVEL WORKER FOR PROCESS POOL -----------------
...
def _scan_worker(args):
    ...
    df_path_str, live_dir_str, scan_symbols, settings_dict, req_dict = args
    df_path = Path(df_path_str)
    ...
    try:
        ...
        # Create settings and strategy inside the worker
        ...
        return {
            ...
            "signal_close": close,
            "entry": h,
            "Target": round(h * (1 + req_dict["target_level"] / 100.0), 2),
            "Stoploss": max(lo, round(h * (1 - req_dict["max_stoploss_pct"] / 100.0), 2)),
            "trend": ltf_trend_str,
...
async def scan_live_trading(req: ScanRequest):
    ...
    worker_args = [
        (str(df_path), str(live_dir), scan_symbols, settings_dict, req_dict)
        for df_path in day_files
    ]
    
    loop = asyncio.get_event_loop()
    # Use ProcessPoolExecutor to bypass the GIL completely and use all available CPU cores
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        tasks = [loop.run_in_executor(executor, _scan_worker, arg) for arg in worker_args]
...
print("api.py updated to use ProcessPoolExecutor!")
