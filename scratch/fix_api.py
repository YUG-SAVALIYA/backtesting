...
# The function starts at @app.post('/api/live-trading/scan')
# We will use regex to replace it until the next symbol resolution endpoint
pattern = re.compile(r'@app\.post\("/api/live-trading/scan"\).*?(?=# ═══════════════════════════════════════════════════════════════════════════════\n# SYMBOL RESOLUTION ENDPOINT)', re.DOTALL)

new_func = """@app.post("/api/live-trading/scan")
async def scan_live_trading(req: ScanRequest):
    ...
    if req.companies.strip().upper() == "ALL":
        scan_symbols = None  # Scan all available files
    else:
        scan_symbols = set(c.strip().strip("'\\"") for c in req.companies.split(",") if c.strip())
    
    # Setup indicators — no filtering at scan level, all rows returned for client-side filtering
    ...
    def process_file(df_path):
        ...
        try:
            ...
            logger.error(f"Error scanning {df_path.name}: {e}")
            return None
