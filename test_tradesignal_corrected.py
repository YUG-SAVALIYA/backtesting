...
# Configure Strategy Settings to MATCH exact user specs:
# Target = 20%, SL = 5%, Book 50% at 10% gain, LTF RSI 65-90, HTF RSI 65-85 (W-FRI), ADX >= 26, Range 3-10%
settings = StrategySettings(
    ...
    signal_candle_range_max_pct=10.0,
...
for sym in symbols:
    ...
    # Correct Weekly aggregation (W-FRI)
    ...
    valid_sigs = []
    for s in sigs:
        sig_date_str = pd.to_datetime(s.entry_time).strftime("%Y-%m-%d")
        if "2026-06-15" <= sig_date_str <= "2026-07-28":
            valid_sigs.append(s)
    ...
    for sig in valid_sigs:
        ...
        if df_5m is not None:
            ...
            for idx, row in day1_5m.iterrows():
                ...
                if bar_time == time(15, 20) or (bar_time > time(15, 20) and entry_bar_price is None):
                    entry_bar_price = row['close']
                    if bar_time == time(15, 20):
                        break
                        
            if is_invalidated or not is_triggered or entry_bar_price is None:
                continue
                
            if entry_bar_price <= sig_high:
                continue
                
            entry_price = entry_bar_price
            max_sl_price = round(entry_price * 0.95, 2)
            stop_loss = max(sig_low, max_sl_price)
            target = round(entry_price * 1.20, 2)  # Corrected to 20% Target
...
print("=== CORRECTED TRADESIGNAL PROJECT RESULTS (20% Target, W-FRI Resampling) ===")
